...
contract TokenHook {
    ...
    // 转账 callback
    function tokenTransferCallback(address to, uint256 amount, bytes calldata data) public returns (bool) {

        transfer(to, amount);

        if (to.code.length > 0) {
            bool res = IERC20Receiver(to).tokensReceived(msg.sender, amount, data);
